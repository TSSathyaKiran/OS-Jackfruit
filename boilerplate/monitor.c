...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        mutex_lock(&monitored_lock);
        {
            ...
            list_for_each_entry(it, &monitored_list, link) {
                if (it->pid == req.pid ||
                    strncmp(it->container_id, req.container_id, sizeof(it->container_id)) == 0) {
                    mutex_unlock(&monitored_lock);
                    kfree(entry);
                    return -EEXIST;
                }
...

    {
        struct monitored_entry *entry;
        struct monitored_entry *tmp;
        bool has_pid = req.pid > 0;
        bool has_id = req.container_id[0] != '\0';

        mutex_lock(&monitored_lock);
        list_for_each_entry_safe(entry, tmp, &monitored_list, link) {
            bool pid_match = has_pid && entry->pid == req.pid;
            bool id_match = has_id && strncmp(entry->container_id,
                                              req.container_id,
...
static void __exit monitor_exit(void)
{
    ...
    while (!list_empty(&monitored_list)) {
        struct monitored_entry *entry =
            list_first_entry(&monitored_list, struct monitored_entry, link);
        list_del(&entry->link);
        kfree(entry);
    }
    mutex_unlock(&monitored_lock);
